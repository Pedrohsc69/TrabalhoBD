...
INSERT INTO Peca (nome, codigo, tipo, fornecedor_id)
VALUES ('Motor 2.0', 'M200', 'Motor', 1),
       ('Pneu Aro 16', 'P16A', 'Pneu', 2),
       ('Freio ABS', 'FABS', 'Freio', 1),
       ('Vidro Temperado', 'VT2024', 'Vidro', 4),
       ('Motor 1.6', 'M1600', 'Motor', 3),
       ('Airbag', 'ABG203', 'Segurança', 1);
-- ____________________________________________________________________
...
-- ____________________________________________________________________
SELECT COUNT(*) FROM VendaCarro;
-- ____________________________________________________________________
SELECT nome, cpf FROM Cliente;
-- ____________________________________________________________________
SELECT Peca.nome, Fornecedor.nome FROM Peca
JOIN Fornecedor ON Peca.fornecedor_id = Fornecedor.id;
-- ____________________________________________________________________
SELECT Carro.modelo, Carro.ano FROM Carro
WHERE linha_producao_id = 1;

SELECT id, nome FROM linhaproducao 
WHERE id = 1;
-- ____________________________________________________________________
SELECT Venda.data, Venda.valor FROM Venda
JOIN Cliente ON Venda.cliente_id = Cliente.id
WHERE Cliente.nome = 'Carlos Mendes';
-- ____________________________________________________________________
SELECT Carro.modelo, Venda.forma_pagamento FROM Carro
JOIN VendaCarro ON Carro.id = VendaCarro.carro_id
JOIN Venda ON VendaCarro.venda_id = Venda.id;
-- ____________________________________________________________________
SELECT Departamento.nome, Departamento.orcamento FROM Departamento
WHERE fabrica_id = 1;
-- ____________________________________________________________________
...
-- ____________________________________________________________________
SELECT Manutencao.data, Manutencao.tipo FROM Manutencao
WHERE carro_id = 1;
-- ____________________________________________________________________
SELECT Peca.nome, PedidoPecas.quantidade FROM PedidoPecas
JOIN Peca ON PedidoPecas.peca_id = Peca.id
WHERE PedidoPecas.id = 1;
-- ____________________________________________________________________
SELECT DISTINCT Fornecedor.nome FROM Fornecedor
JOIN Peca ON Fornecedor.id = Peca.fornecedor_id
JOIN PedidoPecas ON Peca.id = PedidoPecas.peca_id;
-- ____________________________________________________________________
SELECT Carro.modelo, Manutencao.tipo FROM Carro
JOIN Manutencao ON Carro.id = Manutencao.carro_id;
-- ____________________________________________________________________
SELECT Carro.modelo FROM Carro
JOIN VendaCarro ON Carro.id = VendaCarro.carro_id
JOIN Venda ON Venda.id = VendaCarro.venda_id
JOIN Cliente ON Venda.cliente_id = Cliente.id
WHERE Cliente.nome = 'Fernanda Lima';
-- ____________________________________________________________________
SELECT * FROM Venda
...
-- ____________________________________________________________________
SELECT Carro.modelo, Carro.ano FROM Carro
JOIN LinhaProducao ON Carro.linha_producao_id = LinhaProducao.id
JOIN Fabrica ON LinhaProducao.fabrica_id = Fabrica.id
WHERE Fabrica.nome = 'Fábrica Central';
-- ____________________________________________________________________
...
WHERE valor > 90000;
-- ____________________________________________________________________
SELECT SUM(quantidade) FROM PedidoPecas;
-- ____________________________________________________________________
SELECT ContratoVenda.numero, ContratoVenda.condicoes FROM ContratoVenda
JOIN VendaCarro ON ContratoVenda.venda_id = VendaCarro.venda_id
JOIN Carro ON VendaCarro.carro_id = Carro.id
WHERE Carro.chassi = '9BWZZZ377VT004251';
-- ____________________________________________________________________
SELECT Peca.nome FROM PedidoPecas
JOIN Peca ON PedidoPecas.peca_id = Peca.id
WHERE PedidoPecas.status = 'Pendente';
-- ____________________________________________________________________
SELECT tipo, COUNT(*) AS total_manutenções FROM Manutencao
GROUP BY tipo;
